                    ...
                    auto checkLoop = [this, threadId]() {
                        ...
                        if (delayTasks.empty()) {
                            ...
                            for (auto& [cancelableTask, id] : delayTasks) {
                                if (now < cancelableTask->runtime) {
                                    return true;
                                }
                                if (id == 0 && cancelableTask->isActive.load()) {
                                    return false;
                                }
                            }
                            return true;
                            ...
                            if (id == 0) {
                                ...
                                condition.wait_until(lock, cancelableTask->runtime, [this, threadId]() {
                                        ...
                                        for (auto& [cancelableTask, id] : delayTasks) {
                                            ...
                                            if (id == threadId) {
                                                return !cancelableTask->isActive.load();
                                            }
                                        }
                                        return false;
                                    });
                    ...
                    if (tasks.empty()) {
                        ...
                        for (auto it = delayTasks.begin(); it != delayTasks.end(); ++it) {
                            if (it->second == 0) {
                                ...
                                delayTasks.erase(it);
                                lock.unlock();
                                if (task->isActive.compare_exchange_strong(expected, false)) {
                                    (*task)();
                                }
                                break;
                            }
...
thread::ThreadPool &thread::ThreadPool::defaultThreadPool() {
...
void thread::ThreadPool::destroy() {
    impl.reset();
}
